...
/// <summary>
/// Class ClientSocketHandler, that controls a socket_client sender socket.
/// </summary>
public class ClientSocketHandler
{
    ...
    // Since a socket_client can only be connected to ONE server, it will never overwrite data.
    ...
    /// </summary>
    /// <param name="socket_client">Client Socket</param>
    public void Receive(Socket socket_client)
    {
        try
        {
            // Create the state object.  
            StateObject state = new StateObject();
            state.workSocket = socket_client;

            if (socket_client.Connected)
            {
                // Begin receiving the data from the remote device.  
                socket_client.BeginReceive(state.buffer, 0, StateObject.BufferSize, 0,
    ...
    protected virtual void ReceiveCallback(IAsyncResult ar)
    {
        try
        {
            // Retrieve the state object and the socket_client socket from the asynchronous state object.  
